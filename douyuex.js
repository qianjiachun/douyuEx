...
function initPkg_Sign_Renlei() {
    getRenlei();
}

async function getRenlei() {
    await shareAct("20201216rldlmjsx");
    await addFollowRoom("8635723");
    await removeFollowRoom("8635723");
    let ret = await getActRemaining("680");
    if (ret.error == "0") {
        for (let i = 0; i < ret.data.freeCount; i++) {
            let ret2 = await getJackpot("680");
            if (ret2.error == "0") {
                showMessage("【人类跌落梦境】礼盒开启：" + ret2.data.giftName, "success");
            }
        }
    }
}
